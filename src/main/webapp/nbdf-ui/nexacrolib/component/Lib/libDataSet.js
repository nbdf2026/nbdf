...
<Script version="1.0" type="xscript5.1"><![CDATA[var pForm = nexacro.Form.prototype;
...
{	
	...
	if(this.gfn_isNull(sCallbackFunc)) sCallbackFunc = "gfn_callback";
	this.transaction(sSvcID, sSvcURL, sInDataSets, sOutDataSets, sArgument, sCallbackFunc, bAsync, nDataType, bCompress);
};


pForm.gfn_callback = function(sSvcID, nErrCd, sErrMsg)
{
	if(nErrCd < 0) {
		alert("오류 : " + sErrMsg);
		return;
